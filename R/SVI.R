...
svi.fit <- function(X, Y, a, prior_scale = 1.0, sigma2 = 1.0,
                    ...
                    eta = 0.01, K = 100, max_iter = 100,
                    eps = 1e-7, verbose = TRUE) {
  
  n <- nrow(X)
  p <- ncol(X)
  
  # Initialization using ridge regression 
  ...
  for (iter in 1:max_iter) {
    ...
    for (k in 1:K) {
      ...
      for (i in 1:p) {
        if (z_k[i] == 1) {
          grad_mu[i] <- grad_mu[i] + weights[k] * ((theta_k[i] - mu[i]) / sigma1[i]^2)
          grad_sigma[i] <- grad_sigma[i] + weights[k] * (((theta_k[i] - mu[i])^2 - sigma1[i]^2) / sigma1[i]^3)
        }
