...
# Note:
# Choosing higher hyperprior lambda tends to yield better TPR.
...
log_joint <- function(theta, z, Y, X, sigma2, lambda, w) {
  ...
  log_prior_theta <- sum(ifelse(z == 1,
                                log(dlaplace(theta[z == 1], lambda)),
                                0))
...
log_variational <- function(theta, z, mu, sigma1, gamma) {
  ...
  # impossible case: z = 0 but theta != 0 (delta spike)
  ...
  term1 <- if (length(idx1)) {
    sum(log(g[idx1]) +
          dnorm(theta[idx1], mu[idx1], s1[idx1], log = TRUE))
  } else 0
...
svi.fit <- function(X, Y, a,
  ...
  # Initialization via Ridge regression
...
# ------------------------------------------------------------
a_values <- c(0.01, 0.1, 0.25, 0.5, 0.9,
              1.01, 1.1, 1.2, 1.3, 1.5,
              2, 3, 5, 100)
...
for (config in configurations) {
  ...
  for (a in a_values) {
    ...
    metrics_list <- foreach(sim_idx = 1:number_of_simulations,
      ...
      pb$tick()
      sim <- sims[[sim_idx]]
      fit <- suppressWarnings(svi.fit(sim$X, sim$Y, a, prior_scale = 50)) # high lambda
